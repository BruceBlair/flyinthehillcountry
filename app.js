...
const CONFIG = {
  ...
  // Last high-scoring weather/sky event timelapse (shown to external visitors).
  // Generated by sky-watcher when score ≥ 70 burst mode triggers.
  weatherTimelapseManifestUrl: './weather_timelapse_manifest.json',
  mediaBase:  './',
  streamUrl:  'http://192.168.100.202:8892/trackmix_wide/index.m3u8',
  slowmoUrl:  './last_hour_slowmo.mp4',   // fallback if no weather timelapse yet
  refreshMs:  60_000,
...
// ── Weather event timelapse / slow-mo reel (for external visitors) ────────────
// Preference order:
//   1. Last weather/sky event timelapse (score ≥ 70, built by sky-watcher)
//   2. Last-hour slow-motion reel
//   3. Top-photo slideshow
function startSlowmoReel() {
  if (CONFIG.weatherTimelapseManifestUrl) {
    fetch(CONFIG.weatherTimelapseManifestUrl + '?_=' + Date.now())
      .then(r => r.ok ? r.json() : Promise.reject())
      .then(data => {
        const latest = data?.latest;
        if (!latest?.video) return Promise.reject();
        const url = CONFIG.mediaBase + latest.video;
        // Verify the file actually exists before switching the video element.
        return fetch(url, { method: 'HEAD' }).then(r => {
          if (!r.ok) return Promise.reject();
          const score   = latest.score   ? ` · ★ ${latest.score.toFixed(0)}`    : '';
          const frames  = latest.frame_count ? ` · ${latest.frame_count} frames` : '';
          $slideshowImg.classList.add('hidden');
          $liveVideo.classList.remove('hidden');
          $liveVideo.src  = url;
          $liveVideo.loop = true;
          $liveVideo.play().catch(() => {});
          $streamStatus.textContent = `⛅ Last Sky Event${score}${frames}`;
          $streamStatus.classList.add('ok');
        });
      })
      .catch(() => _trySlowmo());
