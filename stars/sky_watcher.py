...
def _read_ptz_pos() -> tuple[float, float, float]:
    """Return (pan, tilt, zoom) from camera. Returns (0.0, 0.0, 1.0) on failure."""
    url = (f"http://{CAMERA_IP}/api.cgi"
           f"?user={CAMERA_USER}&password={CAMERA_PASS}&cmd=GetPtzCtrl")
    try:
        r = requests.post(
            url,
            json=[{"cmd": "GetPtzCtrl", "action": 1, "param": {"channel": 0}}],
            timeout=5,
        )
        ptz = r.json()[0].get("value", {}).get("PtzCtrl", {})
        return float(ptz.get("pan", 0)), float(ptz.get("tilt", 0)), float(ptz.get("zoom", 1))
    except Exception as e:
        log.warning(f"_read_ptz_pos failed: {e}")
        return 0.0, 0.0, 1.0


def _ptz_goto_pan(pan: float) -> None:
    """Absolute-position the pan axis while preserving current tilt and zoom."""
    _, cur_tilt, cur_zoom = _read_ptz_pos()
    url = (f"http://{CAMERA_IP}/api.cgi"
           f"?user={CAMERA_USER}&password={CAMERA_PASS}&cmd=SetAbsolutePtzCtrl")
    try:
        requests.post(
            url,
            json=[{"cmd": "SetAbsolutePtzCtrl", "action": 0,
                   "param": {"channel": 0, "AbsolutePtzCtrl": {
                       "pan": pan, "tilt": cur_tilt, "zoom": cur_zoom,
                       "speed": PANO_SPEED,
                   }}}],
            timeout=5,
        )
    except Exception as e:
        log.warning(f"_ptz_goto_pan failed: {e}")


def _compute_pan_stops(left_pan: float, right_pan: float, n: int) -> list[float]:
    """
    Return n evenly-spaced absolute pan positions spanning the usable range.
    Called once during calibration to compute the fixed stop layout.

    left_pan / right_pan are the mechanical pan limits read at the hard stops.
    Implement the spacing and any edge inset you want here — this is the one
    place that shapes how the stops are laid out across the frame.

    Trade-off: inset = 0 gives maximum coverage but risks pushing against the
    wall if the camera hasn't fully reached the stop yet; a small inset (e.g.
    2–5 % of range) gives a safety margin at the cost of slightly clipping the
    edges of the scene.
    """
    # Fill in your preferred spacing + margin here (~3–5 lines).
    # np is already imported at module level.
    # Example: 3% inset from each edge to avoid pushing against the hard stops:
    #   margin = (right_pan - left_pan) * 0.03
    #   return list(np.linspace(left_pan + margin, right_pan - margin, n))
    margin = (right_pan - left_pan) * 0.03
    return list(np.linspace(left_pan + margin, right_pan - margin, n))

# ── Panorama capture ──────────────────────────────────────────────────────────
def capture_panorama(ts: str) -> Path | None:
    """Sweep camera across its pan range, snap frames, stitch. Returns final JPG path.

    When pan calibration is present, uses absolute positioning across the measured
    range so every frame is evenly distributed — no wall-pegging, no timing drift.
    Falls back to time-based homing + left-step sweep without calibration.
    """
    ...
    pan_left  = calib.get("pan_left")
    pan_right = calib.get("pan_right")

    if pan_left is not None and pan_right is not None:
        pano_stops = _compute_pan_stops(pan_left, pan_right, PANO_SHOTS)
        log.info(f"Panorama: {PANO_SHOTS} stops via absolute pan [{pan_left:.1f}°…{pan_right:.1f}°]")
        frames: list[Path] = []
        for i, pan_pos in enumerate(pano_stops, 1):
            _ptz_goto_pan(pan_pos)
            time.sleep(PANO_SETTLE_SEC)
            ...
            if data:
                ...
                p.write_bytes(data)
                frames.append(p)
                log.info(f"  frame {i:02d}/{PANO_SHOTS}: pan={pan_pos:.1f}°  {len(data)//1024}KB")
    else:
        log.info("Panorama: homing right 20s (no pan calibration)…")
        _ptz("Right")
        time.sleep(20)
        _ptz("Stop")
        time.sleep(2)
...
def _apply_stop_tilt(stop_idx: int, calib: dict) -> None:
    """Move pan to calibrated position (if available), then tilt to calibrated framing."""
    data = calib.get("stops", {}).get(str(stop_idx))
    if not data:
        return

    if "pan" in data:
        _ptz_goto_pan(data["pan"])
        time.sleep(PANO_SETTLE_SEC)
...
def calibrate_scout_tilt() -> None:
    """Two-phase calibration — pan range then tilt framing at each stop.

    Phase 1: pan to each hard edge, read absolute position, compute n evenly-spaced
    stop positions via _compute_pan_stops().  Saves pan_left / pan_right / pan_stops
    so future scouts jump directly to each stop without homing or sequential stepping.

    Phase 2: at each absolute pan stop, sweep tilt from the hard-down reference to
    find the tilt that achieves TILT_CALIB_TARGET sky fraction.

    Trigger:  docker kill --signal=USR2 sky-watcher
              mosquitto_pub -t cameras/commands/calibrate -m now
    """
    log.info(f"=== calibration phase 1: measuring pan range ===")
    calib: dict = {"calibrated_at": datetime.now().isoformat(), "stops": {}}

    # Pan to left edge and read position
    log.info("  homing left…")
    _ptz("Left"); time.sleep(22); _ptz("Stop"); time.sleep(1.5)
    left_pan, _, _ = _read_ptz_pos()
    log.info(f"  left edge: pan={left_pan:.1f}°")

    # Pan to right edge and read position
    log.info("  homing right…")
    _ptz("Right"); time.sleep(22); _ptz("Stop"); time.sleep(1.5)
    right_pan, _, _ = _read_ptz_pos()
    log.info(f"  right edge: pan={right_pan:.1f}°")

    pan_stops = _compute_pan_stops(left_pan, right_pan, SCOUT_POSITIONS)
    log.info(f"  stops ({SCOUT_POSITIONS}): {[round(p, 1) for p in pan_stops]}")
    calib["pan_left"]  = round(left_pan, 2)
    calib["pan_right"] = round(right_pan, 2)
    calib["pan_stops"] = [round(p, 2) for p in pan_stops]

    log.info(f"=== calibration phase 2: tilt at each stop (target sky_frac={TILT_CALIB_TARGET:.2f}) ===")
    for i, target_pan in enumerate(pan_stops):
        log.info(f"  calibrating stop {i + 1}/{SCOUT_POSITIONS} (pan={target_pan:.1f}°)")

        _ptz_goto_pan(target_pan)
        time.sleep(PANO_SETTLE_SEC + 0.5)
        ...
        log.info(f"  → stop {i + 1}: pan={target_pan:.1f}°  tilt_up={best_up_sec:.1f}s  sky={best_frac:.2f}")
    ...
    mqtt_publish("cameras/events/calibration_done", {
        ...
        "pan_left": left_pan, "pan_right": right_pan,
    })
...
def horizon_scout() -> None:
    ...
    When pan calibration is present (run calibrate_scout_tilt() once), each stop
    is reached via SetAbsolutePtzCtrl — no homing, no wall-pegging, no sequential
    stepping.  Without calibration, falls back to time-based pan + home sequence.
    """
    global _tilt_offset_sec
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")

    calib        = _load_tilt_calibration()
    has_pan_calib = bool(calib.get("pan_stops"))

    # Undo any tilt applied during the previous focus dwell
    ...
    # Time-based fallback state (only used when no pan calibration)
    home_op = scan_op = ""
    if has_pan_calib:
        direction = "absolute-pan"
    ...
    for i in range(SCOUT_POSITIONS):
        if has_pan_calib:
            _apply_stop_tilt(i, calib)  # jumps directly to calibrated pan + tilt
        else:
            if i > 0:
    ...
    # Physical pan index: with absolute calibration stop idx == physical idx.
    # With time-based scan, normalise direction-dependent ordering.
    if has_pan_calib or home_op != "Left":
        phys_stops = stops
    else:
    ...
    if has_pan_calib or home_op != "Left":
        ...
        best_phys = SCOUT_POSITIONS - 1 - best_idx
    log.info(f"  best: stop {best_idx+1}  pan_pos={best_phys+1}  score={best_score:.1f}")
    ...
    # Park at best stop — absolute jump with calibration, home+step without
    if has_pan_calib:
        _apply_stop_tilt(best_idx, calib)
    else:
        _ptz(home_op); time.sleep(20); _ptz("Stop"); time.sleep(2)
...
def is_manual_mode() -> bool:
    """Return True if camera_manual_mode is 'on' in Home Assistant."""
    if not HA_TOKEN:
        return False
    try:
        r = requests.get(
            f"{HA_URL}/api/states/input_boolean.camera_manual_mode",
...
def main() -> None:
    ...
    while True:
        ...
        manual = is_manual_mode()
        if manual:
            log.info("Manual mode ON — skipping PTZ movement")

        if score >= SCORE_BURST and data is not None and not manual:
        ...
        elif score >= SCORE_PANO and not manual:
